...
void enemy2_atk::attack(sf::Vector2f charPos, sf::Vector2f enemyPos) {
	...
	if (currentDis <= weapon_range[weapon_class][weapon_rank] && this->scounter == 0 && this->delaycounter == 0) {
		this->distant = 0;
		this->scounter = base_atkSpeed / (weapon_atkSpeed[weapon_class][weapon_rank] / 6);
...
void enemy2_atk::update(int w_class, int w_rank) {
